...
const Allgroups = () => {
  ...
  console.log(groups);
  return (
    <div className="grid groupsClass grid-cols-1 max-h-[80vh] overflow-y-scroll gap-4 md:col-span-6">
      <div className="pr-5 border">
        <form>
          <input
            ...
            className="py-2 mx-3 px-3 rounded-full w-full outline-none my-3 border"
          />
        </form>
      </div>
      <div className="space-y-5">
        {groups.map((item) => (
          <div
            key={item._id}
            className="border w-full gap-5 justify-between flex  p-3"
          >
            <img
            ...
            <Link className="btn" to={`/main/group/${item?._id}`}>
              View Group
            </Link>
          </div>
        ))}
